...
def load_data() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if any(df is None for df in [tx, acc, cust, br]):
        ...
        # Build connection string from env vars (same as previous version)
    ...
    loans_merged = loans.merge(br, on="BranchID", how="left")
...
def demo_dataset() -> pd.DataFrame:
    """Create a small synthetic dataframe that mimics the real schema.
...
if merged_df.empty:
    st.info("Showing demo dataset – real data could not be loaded.")
    merged_df = demo_dataset()
